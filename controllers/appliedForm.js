...
removeButton.addEventListener("click", async () => {
    ...
    const internshipOptions = {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tableName: 'internship' })
      };
    const internshipSurveyOptions = {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tableName: 'internship_survey'})
    };

    alert("Removed Successfully");
    window.location.href = "http://127.0.0.1:5501/views/index.html";
    try {
        // Delete data from internship_survey table first
        await fetch('http://localhost:3000/deleteData', internshipSurveyOptions);
        
        // Wait for the previous fetch to complete before continuing
        await fetch('http://localhost:3000/deleteData', internshipOptions);
    } 
    catch (error) {
        console.log(error);
    }
